...
class Recommender:
    ...
    def predict_per_station(self,station,triptime):
        ...
        headway_pred = np.around(self._headway_model.predict(weather_data),2)
        weather_data['headway'] = headway_pred
        return weather_data.drop(weather_data.columns.difference(['datetime', 'entry', 'exit', 'num_train_running', 'num_train_operational', 'headway']), axis=1)
